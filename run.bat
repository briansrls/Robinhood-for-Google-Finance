...
::In this case, the only library we need is called "requests", so this line is equivalent to "pip install requests".  The library names needed are stored in this text file in case more are needed in the future, you will only need to run one command vs multiple.
